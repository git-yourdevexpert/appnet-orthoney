...
public function orthoney_customer_order_process_ajax_handler() {
    ...
    foreach ($main_orders as $main_order_data) {
        $order_id = $main_order_data->id;
        $main_order_status = $main_order_data->status;
        ...
        // Skip if not matching custom order type
        if (
            ($custom_order_type === 'single_address' && $order_type !== 'Single Address') ||
            ($custom_order_type === 'multiple_address' && $order_type !== 'Multi Address')
        ) {
        ...
        if ($table_order_type === 'main_order') {
            ...
            // Add single order (if applicable)
            if ($order_type === 'Single Address') {
                if($custom_order_status !== 'all'){
                    if ($custom_order_status === $main_order_status) {
                    $filtered_orders[] = $row_data;
                    }
                }
            }

            // Get and render suborders
            $sub_sql = "SELECT * FROM $orders_table WHERE customer_id = %d AND parent_order_id = %d";
            $sub_params = [$user_id, $order_id];
            ...
            foreach ($sub_orders as $sub_data) {
                ...
                // Skip if not matching custom order status
...
public function orthoney_customer_order_process_ajax_handler() {
    ...
    if ($orders) {
        foreach ($orders as $order_data) {
            ...
            foreach ($sub_orders as $sub_data) {
                $sub_order_obj = wc_get_order($sub_data->id);
                if (!$sub_order_obj) continue;

                $sub_created_date = date_i18n(OAM_Helper::$date_format . ' ' . OAM_Helper::$time_format, strtotime($sub_data->date_created_gmt));
                $sub_first_name = $sub_order_obj->get_billing_first_name();
                $sub_last_name = $sub_order_obj->get_billing_last_name();
                $sub_total = wc_price($sub_order_obj->get_total());
                $sub_total_quantity  = 0;
                ...
                $sub_order_rows .= "<tr>
                    <td><div class='thead-data'>Order No</div>" . esc_html($sub_data->id) . "</td>
                    <td><div class='thead-data'>Date</div>" . esc_html($sub_created_date) . "</td>
                    <td><div class='thead-data'>Billing Name</div>" . esc_html($sub_first_name . ' ' . $sub_last_name) . "</td>
                    <td><div class='thead-data'>Affiliate Code</div>" . esc_html($referral_id) . "</td>
                    <td><div class='thead-data'>Total Honey Jar</div>" . esc_html($sub_total_quantity) . "</td>
                    <td><div class='thead-data'>Type</div>" . esc_html($single_order) . "</td>
                    <td class='order-status'><div class='thead-data'>Status</div>" . esc_html($sub_data->status) . "</td>
            ...
            if ($sub_order_rows) {
                $table_content .= '<tr><td colspan="10"><table><thead>
                    <tr>
                        <th>Order No</th><th>Date</th><th>Billing Name</th><th>Affiliate Code</th>
                        <th>Total Honey Jar</th><th>Type</th><th>Status</th>
                        <th style="width:200px">Action</th>
                    </tr></thead><tbody>' . $sub_order_rows . '</tbody></table></td></tr>';
            }
        }
    }
