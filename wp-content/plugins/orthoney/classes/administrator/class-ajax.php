        ...
        if ($offset === 0) {
            ...
            if($sheet_type == 0){
                ...
                fputcsv($full_export_file, [
                    ...
                    'ZipCode', 'Email', 'Phone', 'OrgName', 'DJarPrice', 'ShippingAmount', 'Free Shipping',
                ...
                if($sheet_type == 1){
                    ...
                    foreach ($jar_rows as $jar) {
                        ...
                        $line = [
                            ...
                            OAM_ADMINISTRATOR_Helper::cleanup_pure_text(html_entity_decode(stripslashes($billing_info['first_name']))),
                            OAM_ADMINISTRATOR_Helper::cleanup_pure_text(html_entity_decode(stripslashes($billing_info['last_name']))),
                            trim($billing_info['city']),
                            trim($billing_info['state']),
                            ...
                            trim($billing_info['email']),
                            trim($billing_info['phone']),
                            OAM_ADMINISTRATOR_Helper::cleanup_pure_text(html_entity_decode(stripslashes($row['affiliate_name']))),
                            trim($DJarPriceshow_price),
                            trim($shipping_total),
                            trim($shipping_total == 0 ? "Yes" : "No"),
                            trim($user_id),
                            trim($status),
                            ($order->is_paid() ? 1 : 0),
                            ($order->get_status() === 'on-hold' ? 1 : 0),
                            ($order->is_paid() ? 1 : 0),
                            trim($last_update_date_formatted),
                            trim($order_date),
                            trim($checkout_time),
                            trim($user_type),
                            (ucwords(strtolower(trim($jar['order_type']))) == 'Internal' ? 'UPS' : 'External'),
                            trim($org_activate_status),
                            ((strtolower(trim($row['affiliate_code'])) === strtolower('Honey from the Heart')) ? 'Active' : ($affiliate_status == 1 ? 'Active' : 'Deactivated')),
                        ];
